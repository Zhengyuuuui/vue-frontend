<template>
  <div class="hot-post-list">
    ...
    <div class="max-w-4xl mx-auto mt-6 pb-24">
      ...
      <div v-else-if="hotPosts.length === 0" class="text-center py-10">
        <div class="flex flex-col items-center gap-3">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const fetchHotPosts = async () => {
  ...
  try {
    ...
    if (response && response.success) {
      if (response.data) {
        hotPosts.value = response.data.list || response.data;
        console.log('成功获取热门帖子列表，共', hotPosts.value.length, '条');
      } else {
...
const handleRefreshHotPosts = () => {
  console.log('收到刷新热门帖子列表事件，重新获取热门帖子数据...');
  fetchHotPosts();
};
...
onMounted(() => {
  ...
  window.addEventListener('refreshHotPosts', handleRefreshHotPosts);
});

onUnmounted(() => {
  window.removeEventListener('refreshHotPosts', handleRefreshHotPosts);
});
</script>
